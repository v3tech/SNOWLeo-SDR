 ...
 *
 */
int snowleo_sdr_set_clock(char *i2c_dev, int sample)
{
	int fd, i;

	unsigned char (*snowleo_sdr_Config)[Config_Len_C][Config_Len_L];

	snowleo_sdr_Config = sdr_Config[sample];
...
//			if(snowleo_sdr_i2c_write(fd , SNOWLEO_SDR_I2C_ADDR, (unsigned short)(snowleo_sdr_Config4[i][0]),
//							(unsigned char)snowleo_sdr_Config4[i][1])<0)
