...
int snowleo_sdr_set_tx_dc(int fd, uint8_t dci, uint8_t dcq)
{
	int ret;
	uint8_t tx1[] = {
			(0x42)|(0x1<<7), dci,
...
int snowleo_sdr_set_rx_dc(int fd, uint8_t dci, uint8_t dcq)
{
	int ret;
	uint8_t tx1[] = {
			(0x71)|(0x1<<7), dci|(0x1<<7),
			(0x72)|(0x1<<7), dcq|(0x1<<7),
	};
...
int snowleo_sdr_set_vga(int fd, unsigned int vga, uint8_t channel)
{
	...
	uint8_t tx[8], rx_pll_buffer;
	if(channel == RF_TX_CHANNEL) {
		//printf("set tx vga\n");
		tx[0] = 0x41|(0x1<<7); tx[1] =(uint8_t)(vga&0xff);
		tx[2] = 0x45|(0x1<<7); tx[3] =((uint8_t)((vga>>8)&0xff))<<3;
		tx[4] = 0x44|(0x1<<7); tx[5] =(uint8_t)((vga>>24)&0xff); //PA
		tx[6] = 0x34|(0x1<<7); tx[7] =0x12;
 ...
 *
 */
int snowleo_sdr_spi_init(const char *spi_dev)
{
	int ret = 0;
	int fd;
